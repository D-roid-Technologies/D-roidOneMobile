...
const EventDescriptionScreen: React.FunctionComponent = () => {
  ...
  // );
  // Updated selector to read from the new droidevents slice:
  const registeredEvents = useSelector(
  ...
  // ─────────────────────────────────────────────────────────
  // FIX: Pass isRegistered + registrationData directly to avoid
  // Redux async timing issues where the progress screen reads
  // stale state right after registration completes.
  // ─────────────────────────────────────────────────────────
  const handleRegistrationSubmit = async (formData: any) => {
    ...
    // 3   route params — never relying on Redux state timing.
    if (event?.id === 25 || event?.id === 24) {
      // Small delay so the user sees the modal close cleanly before
    ...
    // 4 Persist + notify in the background (non-blocking)
    try {
  ...
  const handleOpenProgress = () => {
    // If already registered, read fresh data from Redux
    const freshRegistrationData =
      registeredEvents.find((e: any) => e.id === event?.id)?.registrationData ??
      null;
    const freshIsRegistered = registeredEvents.some(
      (e: any) => e.id === event?.id,
    );

    navigation.navigate("EventProgress", {
      event,
      isRegistered: freshIsRegistered,
      registrationData: freshRegistrationData,
    });
  };
